...
int main(int argc, char** argv) {
	...
	count_t points_in_circle_array[num_threads][PAD];
...
#pragma omp parallel num_threads(num_threads)
	{
		...
		points_in_circle_array[omp_get_thread_num()][0] = 0;
...
#pragma omp for
		for(count_t i = 0; i < total_iterations; ++i) {
			...
			if(x * x + y * y <= 1.0f) {
				points_in_circle_array[omp_get_thread_num()][0]++;
			}
		}

#pragma omp atomic
		points_in_circle += points_in_circle_array[omp_get_thread_num()][0];
	}
