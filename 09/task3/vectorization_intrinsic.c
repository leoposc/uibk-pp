...
#define EPSILON 0.0001f     // Tolerance for floating-point comparison

void initialize_vectors(float *a, float *b, float *c, int size) {
...
bool verify_vector(float *a, int size, float expected_value) {
  for (int i = 0; i < size; ++i) {
...
int main(int argc, char *argv[]) {
  ...
  // Allocate memory for the vectors (aligned for SIMD)
  ...
  for (int run = 0; run < REPETITIONS; ++run) {
    for (int i = 0; i < size; i += 4) {
      ...
      __m128 bc_mul = _mm_mul_ps(b_vals, c_vals);
      __m128 result = _mm_add_ps(a_vals, bc_mul);
